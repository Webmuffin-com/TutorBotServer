...
<body>
    ...
    async function sendMessage() {
        try {
            ...
            updateChat(responseOutput, userInput, result); // Update the chat area with the bot's response
    ...
    // Function to handle mixed response formats
    function handleResponse(response) {
        return response.text().then(text => {
            ...
            try {
                ...
                // If we reach here, it's valid XML
                result = {
                    type: 'xml',
                    content: text
                };
            } catch (e) {
                // Not XML, check if it's HTML
    ...
    // Function to update the chat area with user and bot messages
   function updateChat(outputElement, userInput, result) {
        const timestamp = new Date().toLocaleTimeString();
        const userMessage = createMessageHTML('User', userInput, timestamp);

        let botMessage;
        if (result.type === 'xml') {
            const escapedXml = escapeHtml(result.content);
            botMessage = createMessageHTML('Bot', escapedXml, timestamp, true);
        } else if (result.type === 'html') {
            botMessage = createBotHtmlMessage(result.content, timestamp, false);
        } else {
            botMessage = createMessageHTML('Bot', result.content, timestamp, false);
        }

        outputElement.innerHTML += userMessage + botMessage;
        outputElement.scrollTop = outputElement.scrollHeight;
        Prism.highlightAll();
    }

    function createMessageHTML(sender, message, timestamp, isCode = false) {
        const escapedMessage = isCode ? escapeHtml(message) : message;
        const messageContent = isCode ? `<pre><code class="language-markup">${escapedMessage}</code></pre>` : escapedMessage;
        return `<div class="${sender.toLowerCase()}-message"><span class="message-text">${sender}: ${messageContent}</span><span class="timestamp">${timestamp}</span></div>`;
    }

    function createBotHtmlMessage(htmlContent, timestamp) {
        const sandboxId = 'sandbox-' + Date.now(); // Unique ID for the sandbox
        return `
            <div class="bot-message">
                <span class="message-text">Bot:</span>
                <iframe id="${sandboxId}" sandbox="allow-scripts" style="width: 100%; height: 300px; border: 1px solid #ccc;"></iframe>
                <span class="timestamp">${timestamp}</span>
            </div>
        ` + setTimeout(() => {
            const sandboxFrame = document.getElementById(sandboxId);
            sandboxFrame.srcdoc = htmlContent; // Inject the HTML content into the iframe
        }, 0);
    }
