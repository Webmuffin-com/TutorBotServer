...
class TimestampUtils:
    ...
    @staticmethod
    def create_export_timestamp(dt: Optional[datetime] = None, message_type: str = 'bot') -> str:
...
class ConversationFormatter:
        ...
        if len(lines) > 1:
            ...
            if indentations:
                ...
                if most_common_indent >= 4:
                    ...
                    for line in lines[1:]:
                        if line.strip() and len(line) >= most_common_indent and line[:most_common_indent].isspace():
    ...
    @staticmethod
    def format_message(role: str, content: str, message_id: int, message_time: Optional[datetime] = None) -> str:
    ...
    def format_conversation(conversation: List[Tuple[str, str]]) -> str:
        ...
        for i, (role, content) in enumerate(conversation):
            if role in ["user", "assistant"]:
                ...
                message_time = base_time.replace(minute=base_time.minute - ((len(conversation) - i) * 1))
                
                formatted_messages.append(
                    ConversationFormatter.format_message(role, content, message_id, message_time)
                )
                message_id += 1
...
class HTMLConversationExporter:
    ...
    async def generate_conversation_html(
        ...
        conversation = session_cache.m_simpleCounterLLMConversation.get_user_conversation_messages()
